...
export class WebAudioControlsWidgetManager {
    ...
    handleMIDIMessage(event) {
        ...
        for (const w of this.listOfWidgets) {
...
export class WebAudioControlsWidget extends HTMLElement {
    ...
    showtip(d) {
        function valstr(x, c, type) {
            switch (type) {
                ...
                case 'd':
                    return (x | 0).toString()
                case 'f':
                    return Number.parseFloat(x).toFixed(c)
                case 's':
                    return x.toString()
            }
            return ''
        ...
        function numformat(s, x) {
            ...
            for (; j < s.length; ++j) {
                if ('dfxXs'.includes(s[j])) {
                    type = s[j]
                    break
                }
                if (s[j] == '.')
                    m = 1
                else c[m] = c[m] * 10 + Number.parseInt(s[j])
            }
            r = valstr(x, c[1], type)
            if (c[0] > 0)
                r = (`               ${r}`).slice(-c[0])
        ...
        if (this && this.ttframe) {
            if (this.drag || this.hover) {
                ...
                if (s) {
                    ...
                    const rc2 = this.ttframe.getBoundingClientRect()
                    const rc3 = document.documentElement.getBoundingClientRect()
    ...
    setMidiController(channel, cc) {
        if (this.listeningToThisMidiController(channel, cc))
            return
        this.midiController = { channel, cc }
        console.log(
...
export class WebAudioKnob extends WebAudioControlsWidget {
    ...
    connectedCallback() {
      ...
      `
        ...
        if (this.midicc) {
            const ch
                = Number.parseInt(this.midicc.substring(0, this.midicc.lastIndexOf('.'))) - 1
  ...
  <line id="K" x1="32" y1="25" x2="32" y2="11" stroke-linecap="round" stroke-width="6" stroke="${this.coltab[0]}"/>
    ...
    redraw() {
        ...
        if (this && this.elem && this.elem.style) {
            ...
            const sp = this.src ? this.sprites : 100
            if (sp >= 1) {
                const offset = (sp * ratio) | 0
                style.backgroundPosition = `0px ${-offset * this.kh}px`
                style.transform = 'rotate(0deg)'
            }
            else {
                const deg = 270 * (ratio - 0.5)
    ...
    wheel(e) {
        ...
        if (this.log) {
            ...
            if (!e.shiftKey)
                d *= 5
            r += d
            this.setValue(this.min * (this.max / this.min) ** r, true)
        }
        else {
    ...
    pointerdown(ev) {
        ...
        const pointermove = (ev) => {
            ...
            if (this.log) {
                ...
                if (r > 1)
                    r = 1
                this._setValue(this.min * (this.max / this.min) ** r)
            }
            else {
                this._setValue(
                    this.min
...
<rect x="1" y="1" rx="${r}" ry="${r}" width="${this.sw - 2}" height="${
    this.sh - 2
        ...
        this.bodyimg.onload = () => {
            ...
            if (this._width == null)
                this.sw = this.bodyimg.width
            if (this._height == null)
                this.sh = this.bodyimg.height
            if (this.dr == null) {
                if (this.sw > this.sh)
                    this.dr = 'horz'
                else this.dr = 'vert'
            ...
            if (this._knobsrc == null) {
                if (this._knobwidth == null)
                    this.kw = Math.min(this.sw, this.sh)
...
<rect x="3" y="3" width="${this.kw - 6}" height="${
    this.kh - 6
            ...
            this.knobimg.onload = () => {
                ...
                if (this._knobwidth == null)
                    this.kw = this.knobimg.width
                if (this._knobheight == null)
                    this.kh = this.knobimg.height
...
`
    ...
    redraw() {
        ...
        const h2 = this.bheight
        let r = Math.min(8, this.wwidth * 0.2)
        for (let i = this.min, j = 0; i <= this.max; ++i) {
            if (this.kf[i % 12] == 0) {
                const x = this.wwidth * j++ + 1
                if (this.dispvalues.includes(i)) {
                    rrect(
                        this.ctx,
                        x,
                        1,
    ...
    pointerdown(ev) {
        ...
        const pointermove = (ev) => {
            ...
            for (let i = 0; i < p.length; ++i) {
                const px = p[i].clientX - r.left
                const py = p[i].clientY - r.top
                let x, k, ko
    ...
    sendevent() {
        ...
        for (let i = 0, j = this.valuesold.length; i < j; ++i) {
            if (!this.values.includes(this.valuesold[i]))
                notes.push([0, this.valuesold[i]])
        }
        for (let i = 0, j = this.values.length; i < j; ++i) {
            if (!this.valuesold.includes(this.values[i]))
    ...
    setdispvalues(state, note) {
        const n = this.dispvalues.indexOf(note)
        if (state) {
            if (n < 0)
