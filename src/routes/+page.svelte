<script>
	...
	function updateValue(type, value) {
		if (!disabled) {
			...
			let text = property[1]
				? `Updated ${property[0]} ${property[1].toLowerCase()}.`
				: `Updated ${property[0]}.`;
			sendMessage(
				{
					message: "update-" + type,
			...
			if (String(type) != "volume") {
				const propertiesClone = (({ volume, ...key }) => key)(
					properties
				);
	...
	if (browser) {
		...
		chrome.runtime.sendMessage(
			...
			(response) => {
				if (
					["Playing.", "Already playing."].includes(response.message)
				) {
					disabled = false;
...
<div
	class="min-w-[400px] min-h-[500px] opacity-80 bg-gradient-to-tr from-orange-600 via-pink-600 to-purple-700 background-animate"
>
	<div class="grid-main children:(m-2)">
		<div
			class="flex flex-col grid-child-1 bg-green-300/20 rounded-sm children:(rounded-md)"
		>
			<h1 class="text big-text">Audio Presets:</h1>
			{#each presets as preset, i}
				<div
					class={(activePreset === i ? "bg-green-300/20" : "") +
						" mb-1 p-2 hover:(bg-green-300/20)"}
					on:click={() => {
						setPreset(i);
				...
				>
					<input
						class="ml-2 active:(bg-gray-400/20)"
						type="radio"
						name="activePreset"
						{disabled}
						bind:group={activePreset}
						value={i}
					/>
		...
		<div
			class={"flex flex-col justify-center items-center grid-child-2 rounded-md bg-green-300/20 " +
				(properties.pitchWet > 0 ? "bg-green-300/20" : "passive-bg")}
		>
			...
			<input
				...
				step="1"
				{disabled}
				bind:value={properties.pitch}
				on:change={() => {
					updateValue("pitch", properties.pitch);
				}}
			/>
			...
			<input
				...
				step="0.01"
				{disabled}
				bind:value={properties.pitchWet}
				on:change={() => {
					updateValue("pitchWet", properties.pitchWet);
				}}
			/>
		</div>
		<div
			class={"flex flex-col justify-center items-center grid-child-3 rounded-md bg-green-300/20 " +
				(properties.reverbWet > 0 ? "bg-green-300/20" : "passive-bg")}
		>
			...
			<input
				...
				step="0.10"
				{disabled}
				bind:value={properties.reverbDecay}
				on:change={() => {
					updateValue("reverbDecay", properties.reverbDecay);
				}}
			/>
			...
			<input
				...
				step="0.01"
				{disabled}
				bind:value={properties.reverbWet}
				on:change={() => {
					updateValue("reverbWet", properties.reverbWet);
				}}
			/>
		</div>
		<div
			class="flex flex-col justify-center items-center grid-child-4 rounded-md bg-green-300/20"
		>
			...
			<input
				...
				step="0.01"
				{disabled}
				bind:value={properties.volume}
				on:change={() => {
					updateValue("volume", properties.volume);
				}}
			/>
			...
			<input
				...
				step="0.05"
				{disabled}
				bind:value={properties.playbackRate}
				on:change={() => {
					updateValue("playbackRate", properties.playbackRate);
				}}
			/>
			<button
				class="button active:(ring-4 ring-light-200/25) hover:opacity-80"
				{disabled}
				on:click={() => {
					sendMessage({ message: "toggle-media" }, "Played/paused.");
				}}>Play/Pause</button
			>
		</div>
		<div class="flex flex-row justify-between grid-child-5">
			<div class="flex flex-col self-end children:(p-1)">
				...
				<button
					class="button active:(ring-4 ring-light-200/25) hover:opacity-80"
					on:click={exitOptions}>Quit</button
				>
			</div>
			<a
				title="Get help on GitHub!"
				class="self-end hover:opacity-75"
				href="https://github.com/Kernocal/audio-mixer-extension"
				target="_blank"
			>
				<img
					src={GitHub}
					alt=""
					class="float-right w-10 h-10 ml-2 filter-yellow-800"
				/>
			</a>
		</div>
	</div>
